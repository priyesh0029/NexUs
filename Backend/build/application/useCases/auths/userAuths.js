...
const registerUser = async (userData, userRepository, authService) => {
    ...
    const newUser = (0, user_1.user)(name, username, number, email, password ? await authService.encryptPassword(password) : "");
...
const loginUser = async (userData, userRepository, authService) => {
    ...
    return await userRepository.findByProperty(username).then((user) => {
        if (user.length === 0) {
            ...
            throw new appError_1.default(`an error occured while login.please try again after refreshing the page..!`, httpStatus_1.HttpStatus.UNAUTHORIZED);
        }
        return authService
...
//   });
// };
const handleCheckEmail = async (email, userRepository, authService) => {
    return await userRepository.findByProperty(email).then(async (user) => {
        if (user.length === 0) {
            return {
                token: null,
                user,
