...
import ReportedReplies from "./ReportedReplies";
...
const ReportedComments: React.FC<ReportedComments> = ({ selectedTab }) => {
  ...
  const [replyReports, setreplyReports] = useState<IReportedReplies[]>([])
  ...
  const handleTabs = async (activeIndex: number) => {
    if (activeIndex === 0) {
      setreplyOpen(false);
      if (commentReport !== undefined) setTABLE_ROWS(() => commentReport);
    } else if (activeIndex === 1) {
      const response = await getAllRepliesReport();
      console.log("response of reported replies : ", response);
      setreplyReports(response)
      setreplyOpen(true);
    }
  };
                            ...
                            <div className="flex flex-col justify-center items-center group">
                              ...
                              {reports?.length > 0 ? (
                                <Typography
                                  ...
                                  onClick={() => handleReportModal(reports)}
                                >
                                  view
                                </Typography>
                              ) : (
